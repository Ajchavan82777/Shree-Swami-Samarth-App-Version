    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding: iw < 640 ? 0 : '12px' }}
      ...
      <div style={{ background:'var(--dark)', padding:'10px 16px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:6, flexWrap:'wrap' }}>
          ...
          <button onClick={async()=>{ await downloadPDF(printRef.current, invoice.invoice_number||'invoice', orientation); }} title="Download PDF"
...
function InvoiceForm({ onSave, onClose, company, editInvoice }) {
  ...
  const handleFpPrint = () => {
    const style = document.createElement('style');
    style.id = '__fp_print_css';
    style.innerHTML = '@media print { body > * { display:none !important; } #__fp_print_root { display:block !important; position:fixed; top:0;left:0;width:100%;z-index:99999; } }';
    document.head.appendChild(style);
    const div = document.createElement('div');
    div.id = '__fp_print_root';
    div.innerHTML = fpRef.current?.innerHTML || '';
    document.body.appendChild(div);
    window.print();
    setTimeout(() => { document.getElementById('__fp_print_css')?.remove(); document.getElementById('__fp_print_root')?.remove(); }, 1000);
  };
    ...
    {showFullPreview && (
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', zIndex:1100, display:'flex', alignItems:'center', justifyContent:'center', padding:'12px' }}
        ...
        <div style={{ width:'100%', maxWidth:980, height:'94vh', background:'#fff', borderRadius:12, overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 32px 100px rgba(0,0,0,0.6)' }}>
          {/* Toolbar */}
          <div style={{ background:'var(--dark)', padding:'10px 16px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', flexShrink:0 }}>
            <span style={{ color:'var(--gold)', fontWeight:700, fontSize:14 }}>{isEdit ? 'Edit Invoice — Preview' : 'New Invoice — Preview'}</span>
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
              ...
              <button onClick={async () => { await downloadPDF(fpRef.current, 'invoice-preview', orientation); }} style={{ padding:'6px 12px', borderRadius:6, background:'#dc2626', color:'#fff', border:'none', cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', gap:5 }}>
                <Download size={13} /> PDF
              </button>
              <button onClick={async () => { await downloadJPG(fpRef.current, 'invoice-preview'); }} style={{ padding:'6px 12px', borderRadius:6, background:'#2563eb', color:'#fff', border:'none', cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', gap:5 }}>
                <FileImage size={13} /> JPG
              </button>
              <button onClick={() => downloadWord(fpRef.current, 'invoice-preview', 'SSS-INV-PREVIEW')} style={{ padding:'6px 12px', borderRadius:6, background:'#1d4ed8', color:'#fff', border:'none', cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', gap:5 }}>
                <FileText size={13} /> Word
              </button>
              <button onClick={() => setShowFullPreview(false)} style={{ padding:'6px 10px', borderRadius:6, background:'rgba(255,255,255,0.1)', color:'#fff', border:'none', cursor:'pointer' }}><X size={16}/></button>
            </div>
          </div>
          {/* Preview area */}
          <div style={{ flex:1, overflow:'auto', background:'#e5e7eb', padding:24 }}>
            <div style={{ zoom: fpZoom }}>
              <div ref={fpRef} style={{ maxWidth:820, margin:'0 auto', background:'#fff', borderRadius:4, boxShadow:'0 4px 24px rgba(0,0,0,0.15)', overflow:'hidden' }}>
                <InvoicePreview invoice={previewInvoice} template={template} themeName={themeName} font={font} logo={activeLogo} gstType={form.gst_type} company={company} orientation={orientation} margins={margins} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ width:460, flexShrink:0, overflow:'auto', background:'#e5e7eb', padding:16, borderLeft:'1px solid var(--border)', display: tab==='form'&&iw<900?'none':'block' }}>
          ...
          <div style={{ overflow:'auto', WebkitOverflowScrolling:'touch' }}>
            <div style={{ background:'#fff', borderRadius:4, overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,0.1)', minWidth:820 }}>
              <InvoicePreview invoice={previewInvoice} template={template} themeName={themeName} font={font} logo={includeLogo ? (logo || settingsLogo) : ''} gstType={form.gst_type} company={company} orientation={orientation} margins={margins} />
            </div>
          </div>
        </div>
      </div>
